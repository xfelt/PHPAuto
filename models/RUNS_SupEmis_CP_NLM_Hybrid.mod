 ...
 * Modified: Hybrid Tax + Cap Strategy (NLM - CP Optimizer)
 * This model combines emissions tax (in objective) and emissions cap (as constraint)
 * to evaluate cost of carbon reduction: $/ton CO2 saved
 *********************************************/
using CP;
int DEBUG = true;
...
// Program parameters
 ...
 int NB_SUPP = _NBSUPP_;    // 1..NB_SUPP
 string nodeFile = "_NODE_FILE_";
...
//Tree structure index
 ...
 //Objective function: Minimize total cost (including tax) + lead times
 //Hybrid strategy: Tax in objective + Cap as constraint
 ...
 subject to {
 	...
 	ct10: Emis<=EmisCap;  // Emissions cap constraint (active in hybrid)
	forall (i in N){
		forall (j in S){
 ...
 execute {
	writeln("xxxx");  // début du résultat pour php
 	...
 	Result.StCosts = TotalCostTS;  // Total cost including tax
	Result.lts = dlts;
	Result.emiss = Emis;
	write("#Result <fct_obj, tot_cst, tot_ldt, Emiss>:",Result);
	write("#TS:",TotalCostTS);  // Total cost with tax
	write("#CS:",TotalCostCS);   // Cost without tax (for comparison)
	write("#TaxCost:",EmisCost); // Tax cost component
    write("#A:[");
    for (var i in N) {
	   write(a[i]);
       if(i<NB_NODE)write(",");
