/*********************************************
 * Multi-Objective Model - Non-Linear Model (NLM)
...
int obj_primary = _OBJ_PRIMARY_;         // Primary objective: 1=Cost, 2=DIO, 3=WIP, 4=Emissions
...
//Tree structure index
 ...
 dexpr float WIP = sum(i in N)(unit_price[i]*(1+sum(j in S)(z[i][j]*su[i][j]*sup[j][2]))*rqtf[i]*a[i]*x[i]*adup);  // Work In Process (inventory value)
